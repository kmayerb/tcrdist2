...
def create_file( cmds, width, height, filename, create_png = False, background_color=None, use_xlink=False ):
    ...
    if create_png:
        ...
        #basic.convert_svg_to_png( filename, pngfile ) # Oct 15, 2019 COMMENTED THISOUT
...
def protein_logo( upper_left, lower_right, pwm, scale={} ):## scale[pos] should be in range [0,1]
    ...
    for aa in "N1"      :aacolor[aa] = "black" ## for right now, to stop KEYERROR 2019/10/14
        ...
        for freq,aa in l:
            if freq * scaled_height>1: ## at least one pixel high
                ...
                try:
                    ...
                    cmds.append( text_in_box( (x0,y0), (x1,y1), aa, 'black' ) )
            totfreq += freq
