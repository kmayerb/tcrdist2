...
"""
Flexible tools for parralelized computation of pairwise distance.
...
def apply_pw_distance_metric_w_multiprocessing(sequences,
                                               ...
                                               **kwargs):
    """
    The function that computes pairwise distance between a list of sequences,
    using python's multiprocessing package to parralelize the distance compute
    to multiple python interpretors based on the available number of cpu.
    Some explanation is in order.

    The function is pretty flexible, with < f > being mapped to the parrallel
    interpreters taking as input a list of indices (as tuples).
    [(i,j),(i,j), ... (i,j)].

    Here is the default < f > :
    ...
    NOTICE that < f > depends on a distance_wrapper().

    This enables either a user_specified metric function to be supplied via
    the < user_function > arg. In this case, metric must be set to
    'custom.'

    Alternatively, a distance wrapper function can be generated by
    pairwise.function_factory() given a recognized string is provided to the
    metric arg ("nw","bradley"). Further flexibility exists even with a defaults
    metric function as *kwargs may be passed to modify default metric parameters.

    For example, the user can set open and extend penalties and specify the
    substituion matrix for the default reciproval Needleman-Wunsch ('nw') method.
    ...
    f : function
        function that is passed to parralel interpreters. Must accept a list of
...
def function_factory(metric = "nw", **kwargs):
    ...
    kwargs for the API-specifed function if any

    Parameters
    ----------
    metric: string
        string must be "nw", 'hammming'
